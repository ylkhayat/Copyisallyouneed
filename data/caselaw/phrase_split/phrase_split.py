...
if __name__ == '__main__':
    ...
    dataset_path = f'../base_data_{args["chunk_size"]}.txt'
    if args['dataset_path']:
        dataset_path = f"{args['dataset_path']}/base_data_{args['chunk_size']}.txt"
    base_data = load_base_data(dataset_path)
    idx = args['worker_id']
    pkl_file_path = f'../{args["recall_method"]}_search_chunk_{args["chunk_size"]}_{idx}.pkl'
    if args['dataset_path']:
        pkl_file_path = f"{args['dataset_path']}/{args['recall_method']}_search_chunk_{args['chunk_size']}_{idx}.pkl"
    jobs = pickle.load(open(pkl_file_path, 'rb'))
    print(f'[!] collect {len(jobs)} data samples; begin to search for {idx} woker')
    main_search(args, jobs, idx, f"{args['dataset_path']}/{args['recall_method']}_search_result_{args['chunk_size']}_{idx}.txt")
